...
template <class Vertex>
int leftover(const Vertex &start, UnweightedGraph<Vertex> &graph, int maxPorts) {
    std::set<Vertex> visit;

    typedef std::pair<Vertex, int> VertexPair;

    std::queue<VertexPair> p;
    p.push({start, 0});

    while (p.empty() == false) {
        VertexPair vp = p.front();
        p.pop();

        Vertex v = vp.first;
        int portsVisited = vp.second;

        if (visit.find(v) == visit.end()) {
            visit.insert(v);
			
            if (portsVisited < maxPorts) {
                for (Vertex w : graph.getConnectionFrom(v)) {
                    p.push({w, portsVisited + 1});
                }
            }
        }
    }

    return bfs(start, &graph).size() - visit.size();
}
